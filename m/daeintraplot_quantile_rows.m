...
% Parameters for the select_quantile_rows test algorithm.
esize = 2^w; % Size of the codebook (must match codeset's internal esize)
topCandidates = 64; % Number of top-ranked rows passed to select_quantile_rows
...
% select_quantile_rows, in place of the stricter all-runs-faithful test.
...
% select_quantile_rows.
% run: looping variable over runs
% snr: signal-to-noise ratio
% testSet: codebook selected by select_quantile_rows (the algorithm under test)
...
for snr = -3:-6:-9
    ...
    for omegaw = 1:omegawmax
        ...
        % --- Algorithm under test: select_quantile_rows -------------------------
...
% plotset: minimum distance: first col. select_quantile_rows, second col. the
...
if pflag==0
    ...
    legend([h1 h2], {'Select Quantile Rows', 'Original Greedy First'});
    fstring = append(fstring,'rate');
else
    tstring = append(tstring,sprintf('bandwidth, $\\omega_{\\rm w}=%d$',omegaw));
    xlabel('Bandwidth $\omega_{\rm a}$ (Hz)');
    xlim([512 1024]);
    legend([h1 h2], {'Select Quantile Rows', 'Original Greedy First'},'Location', 'northwest');
